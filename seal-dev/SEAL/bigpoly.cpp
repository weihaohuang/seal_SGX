...
namespace seal
{
    ...
    BigPoly::BigPoly() : value_(nullptr), coeffs_(nullptr), coeff_count_(0), coeff_bit_count_(0), coeff_uint64_count_(0), is_alias_(false)
    {
    }

    BigPoly::BigPoly(int coeff_count, int coeff_bit_count) : value_(nullptr), coeffs_(nullptr), coeff_count_(0), coeff_bit_count_(0), coeff_uint64_count_(0), is_alias_(false)
    {
        resize(coeff_count, coeff_bit_count);
    }

    BigPoly::BigPoly(const string &hex_poly) : value_(nullptr), coeffs_(nullptr), coeff_count_(0), coeff_bit_count_(0), coeff_uint64_count_(0), is_alias_(false)
    {
        operator =(hex_poly);
    }

    BigPoly::BigPoly(int coeff_count, int coeff_bit_count, const string &hex_poly) : value_(nullptr), coeffs_(nullptr), coeff_count_(0), coeff_bit_count_(0), coeff_uint64_count_(0), is_alias_(false)
    {
        resize(coeff_count, coeff_bit_count);
        operator =(hex_poly);
    ...
    BigPoly::BigPoly(int coeff_count, int coeff_bit_count, uint64_t *value) : value_(nullptr), coeffs_(nullptr), coeff_count_(0), coeff_bit_count_(0), coeff_uint64_count_(0), is_alias_(false)
    {
        alias(coeff_count, coeff_bit_count, value);
    }

    BigPoly::BigPoly(const BigPoly& copy) : value_(nullptr), coeffs_(nullptr), coeff_count_(0), coeff_bit_count_(0), coeff_uint64_count_(0), is_alias_(false)
    {
    ...
    uint64_t *BigPoly::pointer(int coeff_index)
    {
        ...
        return value_ + coeff_index * coeff_uint64_count_;
    }

    const uint64_t *BigPoly::pointer(int coeff_index) const
    {
        if (coeff_count_ == 0 || coeff_bit_count_ == 0)
        {
            return nullptr;
    ...
    int BigPoly::significant_coeff_count() const
    {
        ...
        return get_significant_coeff_count_poly(value_, coeff_count_, coeff_uint64_count_);
    }
    ...
    {
        return poly_to_hex_string(value_, coeff_count_, coeff_uint64_count_);
    }
    ...
    bool BigPoly::is_zero() const
    {
        ...
        return is_zero_poly(value_, coeff_count_, coeff_uint64_count_);
    }
    ...
    {
        set_zero_poly(coeff_count_, coeff_uint64_count_, value_);
    }
    ...
    void BigPoly::set_zero(int start_coeff, int length)
    {
        ...
        {
            uint64_t *coeff = get_poly_coeff(value_, start_coeff, coeff_uint64_count_);
            set_zero_poly(length, coeff_uint64_count_, coeff);
        }
    }
    ...
    BigPoly &BigPoly::operator =(const string &hex_poly)
    {
        ...
        while (pos < length)
        {
            ...
            hex_string_to_uint(coeff_start, coeff_length, coeff_uint64_count, coeff);
            last_power = power;
        }
    ...
    BigPoly::BigPoly(BigPoly &&source) noexcept : value_(source.value_), coeffs_(source.coeffs_), 
        ...
        coeff_uint64_count_(source.coeff_uint64_count_), is_alias_(source.is_alias_)
    {
