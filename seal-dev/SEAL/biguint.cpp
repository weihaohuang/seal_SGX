...
namespace seal
{
    ...
    {
        int uint64_count = divide_round_up(bit_count_, bits_per_uint64);
        return uint_to_hex_string(value_, uint64_count);
    }

    string BigUInt::to_dec_string() const
    {
        int uint64_count = divide_round_up(bit_count_, bits_per_uint64);
        return uint_to_dec_string(value_, uint64_count, *MemoryPoolMT::default_pool());
    }
    ...
    BigUInt &BigUInt::operator =(const string &hex_value)
    {
        ...
        if (bit_count_ > 0)
        {
            // Copy over value.
            int uint64_count = divide_round_up(bit_count_, bits_per_uint64);
            hex_string_to_uint(hex_value.data(), hex_value_length, uint64_count, value_);
        }
        return *this;
    }
    ...
    BigUInt BigUInt::operator /(const BigUInt& operand2) const
    {
        ...
        MemoryPool &pool = *MemoryPoolMT::default_pool();
