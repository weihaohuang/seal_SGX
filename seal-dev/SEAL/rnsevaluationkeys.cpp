...
namespace seal
{
	void RNSEvaluationKeys::save(std::ostream &stream) const
	{
		// Save the hash block
		stream.write(reinterpret_cast<const char*>(&rns_hash_block_), sizeof(RNSEncryptionParameters::hash_block_type));
...
*/	}
	...
	void RNSEvaluationKeys::load(std::istream &stream)
	{
		// Clear current keys
		keys_.clear();

		// Read the hash block
		stream.read(reinterpret_cast<char*>(&rns_hash_block_), sizeof(RNSEncryptionParameters::hash_block_type));
