...
namespace seal
{
    ...
    void BigPolyArith::multiply(const BigPoly &poly1, const BigPoly &poly2, const BigPoly &poly_mod, const BigUInt &coeff_mod, BigPoly &result)
    {
        ...
        {
             nussbaumer_multiply_poly_poly_coeffmod(poly1ptr.get(), poly2ptr.get(), polymod.coeff_count_power_of_two(), modulus, result.pointer(), pool_);
        }
        else
        {
            nonfft_multiply_poly_poly_polymod_coeffmod(poly1ptr.get(), poly2ptr.get(), polymod, modulus, result.pointer(), pool_);
        }
    }
	...
	void BigPolyArith::negacyclic_shift(const BigPoly & value, int shift, const SmallModulus & smallmodulus, BigPoly & destination)
	{

		resize_destination_if_needed(destination, value.coeff_count(), value.coeff_bit_count());
		negacyclic_shift_poly_smallmod(value.pointer(), value.coeff_count() - 1, shift, destination.pointer(), smallmodulus);
		set_zero_uint(destination.coeff_uint64_count(), destination.pointer() + (value.coeff_count() - 1));
	}
}
