...
namespace seal
{
    BinaryEncoder::BinaryEncoder(const SmallModulus &plain_modulus, const MemoryPoolHandle &pool) : pool_(pool), plain_modulus_(plain_modulus)
    {
        ...
        coeff_neg_threshold_.resize(coeff_neg_threshold_.significant_bit_count());
        ...
        neg_one_.resize(neg_one_.significant_bit_count());
    }
    ...
    void BinaryEncoder::encode(uint64_t value, Plaintext &destination)
    {
        ...
        destination_poly.resize(max(destination_poly.coeff_count(), encode_coeff_count), max(destination_poly.coeff_bit_count(), 1));
        ...
        while (value != 0)
        {
            if ((value & 1) != 0)
            {
                destination_poly[coeff_index] = 1;
            }
            value >>= 1;
            coeff_index++;
        }
    ...
    void BinaryEncoder::encode(int64_t value, Plaintext &destination)
    {
        ...
        if (value < 0)
        {
            ...
            destination_poly.resize(max(destination_poly.coeff_count(), encode_coeff_count), max(destination_poly.coeff_bit_count(), encode_coeff_bit_count));
            destination_poly.set_zero();
            ...
            while (pos_value != 0)
            {
                if ((pos_value & 1) != 0)
                {
                    destination_poly[coeff_index] = neg_one_;
                }
                pos_value >>= 1;
                coeff_index++;
            }
    ...
    int64_t BinaryEncoder::decode_int64(const Plaintext &plain)
    {
        ...
        int sig_uint64_count = plain_modulus_.uint64_count();
        Pointer pos_value(allocate_uint(sig_uint64_count, pool_));

        // Determine coefficient threshold for negative numbers.
        int coeff_neg_threshold_bits = coeff_neg_threshold_.bit_count();

        int64_t result = 0;
        for (int bit_index = plain_poly.significant_coeff_count() - 1; bit_index >= 0; bit_index--)
        {
            const BigUInt &coeff = plain_poly[bit_index];
            ...
            {
                if (sub_uint_uint(plain_modulus_.pointer(), plain_modulus_.uint64_count(), coeff.pointer(), sig_uint64_count, false, sig_uint64_count, pos_value.get()))
                {
    ...
    BalancedEncoder::BalancedEncoder(const SmallModulus &plain_modulus, uint64_t base, const MemoryPoolHandle &pool) : pool_(pool), plain_modulus_(plain_modulus), base_(base)
    {
        if (base <= 2)
        {
    ...
    void BalancedEncoder::encode(uint64_t value, Plaintext &destination)
    {
        ...
        destination_poly.resize(max(encode_coeff_count, destination_poly.coeff_count()), plain_modulus_.bit_count());
        destination_poly.set_zero();
        ...
        while (value != 0)
        {
            ...
            {
                destination_poly[coeff_index] = remainder;
            }
            else if (remainder > (base_ - 1) / 2)
            {
                destination_poly[coeff_index] = base_ - remainder;
                sub_uint_uint(plain_modulus_.pointer(), plain_modulus_.uint64_count(), destination_poly[coeff_index].pointer(), destination_poly[coeff_index].uint64_count(), false, destination_poly[coeff_index].uint64_count(), destination_poly[coeff_index].pointer());
            }
    ...
    void BalancedEncoder::encode(const BigUInt &value, Plaintext &destination)
    {
        ...
        int dest_coeff_uint64_count = destination_poly.coeff_uint64_count();
        ...
        while (!is_zero_uint(temp.get(), value.uint64_count()))
        {
            ...
            uint64_t *dest_coeff = get_poly_coeff(destination_poly.pointer(), coeff_index, dest_coeff_uint64_count);
    ...
    BinaryFractionalEncoder::BinaryFractionalEncoder(const SmallModulus &plain_modulus, const BigPoly &poly_modulus, int integer_coeff_count, int fraction_coeff_count, const MemoryPoolHandle &pool) 
    ...
    Plaintext BinaryFractionalEncoder::encode(double value)
    {
        ...
        Plaintext encoded_int(BigPoly(poly_modulus_.coeff_count(), encoder_.plain_modulus().bit_count()));
        ...
        int plain_uint64_count = divide_round_up(encoder_.plain_modulus().bit_count(), bits_per_uint64);
        ...
        Plaintext result(BigPoly(poly_modulus_.coeff_count(), encoder_.plain_modulus().bit_count()));
        add_poly_poly(encoded_int.get_poly().pointer(), encoded_fract.get(), poly_modulus_.coeff_count(), plain_uint64_count, result.get_poly().pointer());
    ...
    double BinaryFractionalEncoder::decode(const Plaintext &plain)
    {
        ...
        if (plain_poly.significant_coeff_count() >= poly_modulus_.significant_coeff_count())
        {
            throw invalid_argument("plain does not represent a valid plaintext polynomial");
        }

        int coeff_uint64_count = divide_round_up(plain_poly.coeff_bit_count(), bits_per_uint64);

        // Extract the fractional and integral parts
        BigPoly encoded_int(poly_modulus_.coeff_count() - 1 - fraction_coeff_count_, plain_poly.coeff_bit_count());
        BigPoly encoded_fract(poly_modulus_.coeff_count() - 1 - integer_coeff_count_, plain_poly.coeff_bit_count());

        // Integer part
        set_poly_poly(plain_poly.pointer(), integer_coeff_count_, coeff_uint64_count, encoded_int.pointer());

        // Read from the top of the poly all the way to the top of the integral part to obtain the fractional part
        set_poly_poly(plain_poly[integer_coeff_count_].pointer(), poly_modulus_.coeff_count() - 1 - integer_coeff_count_, coeff_uint64_count, encoded_fract.pointer());
    ...
    Plaintext BalancedFractionalEncoder::encode_odd(double value)
    {
        ...
        for (int i = 0; i < fraction_coeff_count_; ++i)
        {
            ...
            {
                sub_uint_uint(encoder_.plain_modulus_.pointer(), encoder_.plain_modulus_.uint64_count(), encoded_fract.get(), plain_uint64_count, false, plain_uint64_count, encoded_fract.get());
            }
    ...
    Plaintext BalancedFractionalEncoder::encode_even(double value)
    {
        ...
        for (int i = fraction_coeff_count_ - 1; i >= 0; --i)
        {
            ...
            {
                sub_uint_uint(encoder_.plain_modulus_.pointer(), encoder_.plain_modulus_.uint64_count(), encoded_fract_ptr, plain_uint64_count, false, plain_uint64_count, encoded_fract_ptr);
            }
            encoded_fract_ptr -= plain_uint64_count;
        }
    ...
    void IntegerEncoder::encode(uint64_t value, Plaintext &destination)
    {
        ...
        BigPoly &destination_poly = destination.get_poly();
        destination_poly.resize(destination_poly.significant_coeff_count(), encoder_->plain_modulus().bit_count());
    }
