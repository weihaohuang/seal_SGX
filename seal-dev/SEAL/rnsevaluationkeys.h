...
namespace seal
{
	...
	@par Thread Safety
	In general, reading from RNSEvaluationKeys is thread-safe as long as no other thread is concurrently
	...
	*/
	...
	public:
		...
		constructor.
		*/
		RNSEvaluationKeys() = default;

		/**
		Creates a new RNSEvaluationKeys by copying an old one.

		@param[in] copy The RNSEvaluationKeys to copy from
		*/
		RNSEvaluationKeys(const RNSEvaluationKeys &copy) = default;

		/**
		Creates a new RNSEvaluationKeys by movin an old one.

		@param[in] source The RNSEvaluationKeys to move from
		*/
		RNSEvaluationKeys(RNSEvaluationKeys &&source) = default;

		/**
		Copies an old RNSEvaluationKeys to the current one.

		@param[in] assign The RNSEvaluationKeys to copy from
		*/
		RNSEvaluationKeys &operator =(const RNSEvaluationKeys &assign) = default;

		/**
		Moves an old RNSEvaluationKeys to the current one.

		@param[in] assign The RNSEvaluationKeys to move from
		*/
		RNSEvaluationKeys &operator =(RNSEvaluationKeys &&assign) = default;
		...
		*/
		...
		/**
		Saves the RNSEvaluationKeys instance to an output stream. The output is in binary
		format and not human-readable. The output stream must have the "binary" flag set.

		@param[in] stream The stream to save the RNSEvaluationKeys to
		@see load() to load a saved RNSEvaluationKeys instance.
		*/
		void save(std::ostream &stream) const;

		/**
		Loads an RNSEvaluationKeys instance from an input stream overwriting the current
		RNSEvaluationKeys instance.

		@param[in] stream The stream to load the RNSEvaluationKeys instance from
		@see save() to save an RNSEvaluationKeys instance.
		*/
		...
		*/
		const std::vector<std::vector<std::pair<BigPolyArray, BigPolyArray> > > &get_data() const
		{
			return keys_;
		}
		...
		*/
		...
		/**
		Enables access to private members of seal::RNSEvaluationKeys for .NET wrapper.
		*/
		struct RNSEvaluationKeysPrivateHelper;
	...
	private:
		...
		*/
		inline std::vector<std::vector<std::pair<BigPolyArray, BigPolyArray> > > &get_mutable_data()
		{
			return keys_;
		}
		...
		*/
		std::vector<std::vector<std::pair<BigPolyArray, BigPolyArray> > > keys_;
