...
namespace seal
{
    ...
    {
        /**
        NTT for sequences of elements in extension fields Z_{p}[x]/(f(x)).
        ...
        */
        ...
        {
        public:
            ExFieldNTT(std::shared_ptr<ExField> ex_field, int coeff_count_power);

            void ntt_negacyclic(std::vector<ExFieldElement> &sequence);

            void inverse_ntt_negacyclic(std::vector<ExFieldElement> &sequence);

        private:
            bool generate();

            ExFieldElement compute_primitive_root();

            ExFieldElement inv_primitive_root(ExFieldElement root);

            void ntt_powers_of_primitive_root(const ExFieldElement &root, std::vector<ExFieldElement> &powers);
            ...
            std::vector<ExFieldElement> root_powers_;

            std::vector<ExFieldElement> inv_root_powers_;
