...
namespace seal
{
    namespace util
    {
        ...
        std::string poly_to_hex_string(const std::uint64_t *value, int coeff_count, int coeff_uint64_count);
        
        class MemoryPool;
        std::string uint_to_dec_string(const std::uint64_t *value, int uint64_count, MemoryPool &pool);

        std::string poly_to_dec_string(const std::uint64_t *value, int coeff_count, int coeff_uint64_count, MemoryPool &pool);

        void hex_string_to_uint(const char *hex_string, int char_count, int uint64_count, std::uint64_t *result);

        int get_hex_string_bit_count(const char *hex_string, int char_count);

        int get_power_of_two(std::uint64_t value);

		int get_close_power_of_two(std::uint64_t value);
		...
		int HW(std::uint64_t x);

		std::uint64_t optimal_slpit(std::uint64_t x);
