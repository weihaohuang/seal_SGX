...
namespace seal
{
    namespace util
    {
        ...
        void HashFunction::sponge_absorb(const uint64_t input[sha3_rate_uint64_count], sha3_state_type &state)
        {
            for (uint8_t x = 0; x < 5; x++)
            {
                for (uint8_t y = 0; y < 5; y++)
                {
                    uint8_t index = 5 * y + x;
                    state[x][y] ^= index < sha3_rate_uint64_count ? input[index] : 0ULL;
                }
            }
            keccak_1600(state);
        }

        void HashFunction::sponge_squeeze(const sha3_state_type &sha3_state, sha3_block_type &sha3_block)
        {
        ...
        void HashFunction::sha3_hash(const uint64_t *input, int uint64_count, sha3_block_type &sha3_block)
        {
            ...
            Pointer padded_input(MemoryPoolMT::default_pool()->get_for_uint64_count(padded_uint64_count));
