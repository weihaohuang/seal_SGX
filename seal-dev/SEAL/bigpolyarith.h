...
namespace seal
{
    ...
    */
    ...
    public:
        /**
        Creates a BigPolyArithm instance. Optionally, the user can give a reference to a MemoryPoolHandle 
        object to use a custom memory pool instead of the global memory pool (default).
        */
        BigPolyArith(const MemoryPoolHandle &pool = MemoryPoolHandle::acquire_global()) : pool_(pool)
        {
        }
        ...
        */
		...
		void negacyclic_shift(const BigPoly &value, int shift, const SmallModulus &smallmodulus, BigPoly & destination);
