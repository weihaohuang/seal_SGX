...
namespace seal
{
	...
	vector<uint64_t> compute_lift_uint64(long p, long n) {
		...
		Modulus power_of_n_plus_one_(power_of_n_plus_one.pointer(), power_of_n_plus_one.uint64_count());

		// Set result = X^p
		for (int i = 0; i < p; i++) result = bigpolyarith.multiply(result, X);
		...
		for (int i = 1; i < n + 1; i++)
		{
			...
			modulo_poly_coeffs(fi.pointer(), fi.coeff_count(), power_of_n_plus_one_, pool_);
			power_of_i_fi.resize(fi.coeff_count(), fi.coeff_bit_count() + power.bit_count());
			multiply_poly_scalar_coeffmod(fi.pointer(), fi.coeff_count(), power.pointer(), power_of_n_plus_one_, power_of_i_fi.pointer(), pool_);
			sub_poly_poly_coeffmod(result.pointer(), power_of_i_fi.pointer(), power_of_i_fi.coeff_count(), power_of_n_plus_one.pointer(), power_of_i_fi.coeff_uint64_count(), result.pointer());
		}
		vector<uint64_t> lift;
		lift.resize(result.coeff_count());
		for (long i = 0; i < result.coeff_count(); i++) {
			lift[i] = *(result[i].pointer());
		}
		return lift;
	}
	...
	vector<uint64_t> compute_remainlsd_uint64(long p, long n) {
		...
		for (int i = 0; i < n; i++)	power_of_n *= p;
		Modulus mod_(power_of_n.pointer(), power_of_n.uint64_count());
		MemoryPoolHandle pool_;
		...
		multiply_poly_scalar_coeffmod(prod.pointer(), prod.coeff_count(), coeff.pointer(), mod_, amcoeff_times_prod.pointer(), pool_);
	...
	BigPoly fpoly_ep(long base, long exponent, long n)
	{
		...
		BigUInt power_of_n_plus_one("1");
		for (int i = 0; i < n + 1; i++) power_of_n_plus_one *= p;
		Modulus mod_(power_of_n_plus_one.pointer(), power_of_n_plus_one.uint64_count());
		...
		for (int i = 0; i < p; i++)
		{
			...
			multiply_poly_scalar_coeffmod(prod.pointer(), prod.coeff_count(), inv.pointer(), mod_, inv_times_prod.pointer(), pool_);
    ...
    int optimal_parameter_paterson(int degree)
    {
        ...
        int min_k;
        int min_m;
        int upper_bound = 2 * (int)(sqrt((double)degree));
