...
namespace seal
{
    ...
    RNSPolyCRTBuilder::RNSPolyCRTBuilder(const RNSContext &context, const MemoryPoolHandle &pool) :
    ...
    RNSPolyCRTBuilder::RNSPolyCRTBuilder(const RNSPolyCRTBuilder &copy) :
    ...
    void RNSPolyCRTBuilder::populate_roots_of_unity_vector()
    {
        int coeff_uint64_count = parms_.plain_modulus().uint64_count();

        Pointer generator_sq(allocate_uint(coeff_uint64_count, pool_));
        multiply_uint64_smallmod(ntt_tables_.get_root(), ntt_tables_.get_root(), mod_, generator_sq.get());
        ...
        {
            multiply_uint64_smallmod(destination_ptr, generator_sq.get(), mod_, destination_ptr + coeff_uint64_count);
            destination_ptr += coeff_uint64_count;
        }
    }

    void RNSPolyCRTBuilder::compose(vector<uint64_t> values, Plaintext &destination)
    {
    ...
    void RNSPolyCRTBuilder::compose(const vector<BigUInt> &values, Plaintext &destination)
    {
        ...
        inverse_smallntt_negacyclic_harvey(destination_poly.pointer(), ntt_tables_, pool_);
    }

    void RNSPolyCRTBuilder::decompose(const Plaintext &plain, vector<BigUInt> &destination)
    {
        ...
        smallntt_negacyclic_harvey(poly_copy.get(), ntt_tables_, pool_);
