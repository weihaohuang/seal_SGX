    ...
    char* BigPolyArray::save(int &buffer_size)
  	{
  		...
  		offset += sizeof(int32_t);
  		memcpy(buffer + offset, (char*)value_.get(), size_ * coeff_count_ * coeff_uint64_count() * bytes_per_uint64);
  	...
  	void BigPolyArray::load(char* buffer)
  	{
  		...
  		memcpy(value_.get(), buffer + offset, size_ * coeff_count_ * coeff_uint64_count() * bytes_per_uint64);
