    ...
    char* BigPoly::save(int &buffer_size)
  	{
  		...
  		offset += sizeof(int32_t);
  		memcpy(buffer + offset, (char*)value_.get(), coeff_count_ * coeff_uint64_count * bytes_per_uint64);
  		return buffer;
  	}
  	...
  	void BigPoly::load(char* buffer)
  	{
  		...
  		if (read_coeff_uint64_count == coeff_uint64_count)
  		{
  			memcpy(value_.get(), buffer + offset, read_coeff_count * coeff_uint64_count * bytes_per_uint64);
