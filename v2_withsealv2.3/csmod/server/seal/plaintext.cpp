...
namespace seal
{
    ...
    char* Plaintext::save(int &buffer_size)
  	{
  		int32_t coeff_count32 = static_cast<int32_t>(coeff_count_);
  		  
  		char *buffer = (char*)malloc(sizeof(int32_t) + coeff_count_ * bytes_per_uint64 + 1);
  		buffer_size = sizeof(int32_t) + coeff_count_ * bytes_per_uint64 + 1;
  
  		int offset = 0;
  		memcpy(buffer + offset, (char*)&coeff_count32, sizeof(int32_t));
  		offset += sizeof(int32_t);
  		
  		memcpy(buffer + offset, (char*)(plaintext_poly_.get()), coeff_count_ * bytes_per_uint64);
  		return buffer;
  	}
  
  	void Plaintext::load(char* buffer)
  	{
  		int offset = 0;
