...
namespace seal
{
    ...
    char* Ciphertext::save(int &buffer_size)
    {
  		  int32_t size32 = static_cast<int32_t>(size_);
        int32_t poly_coeff_count32 = static_cast<int32_t>(poly_coeff_count_);
        int32_t coeff_mod_count32 = static_cast<int32_t>(coeff_mod_count_);
        
        char *buffer = (char*)malloc(sizeof(EncryptionParameters::hash_block_type) + sizeof(int32_t)*3 + size_ * poly_coeff_count_ * coeff_mod_count_ * bytes_per_uint64 + 1);
        buffer_size = sizeof(EncryptionParameters::hash_block_type) + sizeof(int32_t)*3 + size_ * poly_coeff_count_ * coeff_mod_count_ * bytes_per_uint64 + 1;
        ...
        memcpy(buffer + offset, (char*)(&size32), sizeof(int32_t));
        offset += sizeof(int32_t);
        
        memcpy(buffer + offset, (char*)(&poly_coeff_count32), sizeof(int32_t));
        offset += sizeof(int32_t);
        
        memcpy(buffer + offset, (char*)(&coeff_mod_count32), sizeof(int32_t));
        offset += sizeof(int32_t);
        
        memcpy(buffer + offset, (char*)(ciphertext_array_.get()), size_ * poly_coeff_count_ * coeff_mod_count_ * bytes_per_uint64);
        
        return buffer;
    }
    
    void Ciphertext::load(char* buffer)
    {
        int offset = 0;
        ...
        int32_t read_size32 = 0;
        memcpy(&read_size32, buffer + offset, sizeof(int32_t));
        offset += sizeof(int32_t);
    
        int32_t read_poly_coeff_count32 = 0;
        memcpy(&read_poly_coeff_count32, buffer + offset, sizeof(int32_t));
        offset += sizeof(int32_t);
        
        int32_t read_coeff_mod_count32 = 0;
        memcpy(&read_coeff_mod_count32, buffer + offset, sizeof(int32_t));
        offset += sizeof(int32_t);
        ...
        memcpy(ciphertext_array_.get(), buffer + offset, size_ * poly_coeff_count_ * coeff_mod_count_ * bytes_per_uint64);
        // Read data
        //stream.read(reinterpret_cast<char*>(ciphertext_array_.get()), size_ * poly_coeff_count_ * coeff_mod_count_ * bytes_per_uint64);
    }
